...
The [territory-fusion research](https://github.com/orbis-geonet/research) makes this concrete. Communities stake claims to real-world places, and thousands of claims fuse in real time into organic tribal polygons — tangent bridges connect nearby claims, rival collisions reject a merge, and a hole-preserving union keeps interior gaps intact. A territory is addressed by the claims it contains; fusion and split are idempotent. Ground is **held by presence, not decree**: every check-in grows a claimed place's circle, a claim that overlaps a rival place carves that circle down by exactly the overlap, and places no one revisits decay to nothing — out-claim a tribe at its own place and the ground changes hands.
...
| [user-management-dashboard](https://github.com/orbis-geonet/user-management-dashboard) | Internal admin dashboard the Orbis team uses to operate the network — manage users, tribes, places and posts, moderate flagged content, track growth statistics, and run email campaigns. Spring Boot + MongoDB REST API with a [react-admin](https://marmelab.com/react-admin/) SPA. |
